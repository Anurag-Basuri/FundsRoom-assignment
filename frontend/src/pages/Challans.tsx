...
const Challans = () => {
  ...
  const handleConfirm = async (id: string) => {
    if (
      confirm(
        "Are you sure you want to confirm this challan? This will deduct stock.",
      )
    ) {
      try {
        ...
        toast.error(
          error.response?.data?.message || "Failed to confirm challan",
        );
      }
    }
  };
  ...
  const handleCancel = async (id: string) => {
    if (confirm("Are you sure you want to cancel this challan?")) {
      try {
        ...
        toast.error(
          error.response?.data?.message || "Failed to cancel challan",
        );
      }
    }
  };
            ...
            {isLoading ? (
              ...
              data?.data.map((challan) => (
                <TableRow key={challan.id}>
                  ...
                  <TableCell>
                    {format(new Date(challan.created_at), "dd MMM yyyy")}
                  </TableCell>
                  <TableCell>
